...
%% you can develop and deploy applications faster. Amazon Virtual Private
%% Cloud (Amazon VPC) enables you to provision a logically isolated section
%% of the Amazon Web Services Cloud where you can launch Amazon Web Services
%% resources in a virtual network that you've defined. Amazon Elastic
%% Block Store (Amazon EBS) provides block level storage volumes for use with
%% EC2 instances. EBS volumes are highly available and reliable storage
%% volumes that can be attached to any running instance and used like a hard
%% drive.
%%
...
%% attachment requests. Use `RejectTransitGatewayVpcAttachment' to reject
...
%% `pending-acceptance' state, and you must be the owner of the peer VPC.
...
%% instance or network interface. After you release an Elastic IP address, it
%% is released to the IP address pool and can be allocated to a different
...
%%
%% This action replaces the existing security groups with the specified
%% security groups.
...
%% instances that are in subnets in Wavelength Zones) with an instance or a
%% network interface.
%%
...
%% to more than one VPC at a time. You can only link an instance that's
%% in the `running' state. An instance is automatically unlinked from a
%% VPC when it's stopped - you can link it to the VPC again when you
%% restart it.
%%
...
%%
%% </li> <li> Amazon Web Services Marketplace product codes are copied from
...
%%
%% Ingress authorization rules act as firewall rules that grant access to
%% networks. You must configure ingress authorization rules to enable clients
...
%% your security group in a VPC, the `IpPermissions' must include a
...
%% rule for your security group in a VPC, the `IpPermissions' must
...
%%
%% <ul> <li> The Capacity Reservation Fleet's status changes to
%% `cancelled'.
%%
%% </li> <li> The individual Capacity Reservations in the Fleet are
%% cancelled. Instances running in the Capacity Reservations at the time of
%% cancelling the Fleet continue to run in shared capacity.
%%
%% </li> <li> The Fleet stops creating new Capacity Reservations.
%%
%% </li> </ul>
...
%% all artifacts of the conversion, including a partially uploaded volume or
%% instance. If the conversion is complete or is in the process of
...
%% partially-created Amazon S3 objects. If the export task is complete or is
...
%% useful when a product code owner must verify whether another user's
...
%% `Encrypted' during the copy operation. You cannot create an
...
%% Outpost using DestinationOutpostArn. Backing snapshots copied to an
...
%% Amazon S3.
%%
%% You can copy a snapshot within the same Region, from one Region to
%% another, or from a Region to an Outpost. You can't copy a snapshot
...
%%
%% A Client VPN endpoint is the resource you create and configure to enable
%% and manage client VPN sessions. It is the destination endpoint at which
...
%% destination network routes. Each route in the route table specifies the
%% path for traﬃc to speciﬁc resources or networks.
...
%% @doc Creates a range of customer-owned IP addresses.
...
%% connection. You must provide the IP address of the customer gateway
...
%% commas. To have your instance receive a custom DNS hostname as specified
%% in `domain-name', you must set `domain-name-servers' to a custom
%% DNS server.
%%
...
%% domain name (for example, `ExampleCompany.com'). This value is used to
%% complete unqualified DNS hostnames. Important: Some Linux operating
...
%%
%% </li> <li> `netbios-name-servers' - The IP addresses of up to four
...
%% automate your IP address management workflows including assigning,
...
%% and in the specified PEM or PPK format.
%%
...
%% with PuTTY. If a key with the specified name already exists, Amazon EC2
%% returns an error.
%%
...
%%
%% You can specify an existing version of launch template from which to base
%% the new version.
%%
...
%%
%% <ul> <li> `LocalGatewayVirtualInterfaceGroupId'
%%
%% </li> <li> `NetworkInterfaceId'
%%
%% </li> </ul>
...
%% @doc Creates a local gateway route table.
...
%% @doc Creates a local gateway route table virtual interface group
%% association.
...
%%
%% You can specify one or more entries for the prefix list. Each entry
...
%%
%% This action creates a network interface in the specified subnet with a
%% private IP address from the IP address range of the subnet. You can create
%% either a public NAT gateway or a private NAT gateway.
%%
...
%%
%% The strategy of the placement group determines how the instances are
...
%% a new volume that is restored to the original root volume's launch
...
%% list of your Standard Reserved Instances, you can use the
...
%%
%% For an IPv4 only subnet, specify an IPv4 CIDR block. If the VPC has an
%% IPv6 CIDR block, you can create an IPv6 only subnet or a dual stack subnet
%% instead. For an IPv6 only subnet, specify an IPv6 CIDR block. For a dual
%% stack subnet, specify both an IPv4 CIDR block and an IPv6 CIDR block.
%%
%% A subnet CIDR block must not overlap the CIDR block of an existing subnet
%% in the VPC. After you create a subnet, you can't change its CIDR
%% block.
%%
...
%% reserves both the first four and the last IPv4 address in each
%% subnet's CIDR block. They're not available for your use.
%%
...
%% policy. For example, you can group all Verified Access instances
%% associated with &quot;sales&quot; applications together and use one common
...
%%
%% When an application request is made, the identity information sent by the
%% trust provider is evaluated by Verified Access before allowing or denying
...
%%
%% A VPC endpoint provides a private connection between the specified VPC and
...
%%
%% The accepter VPC can belong to another Amazon Web Services account and can
...
%%
%% You must disassociate all target networks before you can delete a Client
%% VPN endpoint.
...
%% been automatically added, disassociate the target subnet from the Client
%% VPN endpoint.
...
%% @doc Deletes a range of customer-owned IP addresses.
...
%% the `deleted_terminating' state. Otherwise, the EC2 Fleet enters the
...
%% == Restrictions ==
%%
%% <ul> <li> You can delete up to 25 `instant' fleets in a single
%% request. If you exceed this number, no `instant' fleets are deleted
%% and an error is returned. There is no restriction on the number of fleets
%% of type `maintain' or `request' that can be deleted in a single
%% request.
%%
...
%%
%% You must detach the internet gateway from the VPC before you can delete
%% it.
...
%% the only version for the launch template, you must delete the entire
...
%% @doc Deletes a local gateway route table.
...
%% @doc Deletes a local gateway route table virtual interface group
%% association.
...
%%
%% Deleting a public NAT gateway disassociates its Elastic IP address, but
%% does not release the address from your account. Deleting a NAT gateway
...
%%
%% By default, you cannot delete the permission if the account for which
%% you're removing the permission has attached the network interface to
%% an instance. However, you can force delete the permission, regardless of
%% any attachment.
...
%%
%% You must disassociate the route table from any transit gateway route
%% tables before you can delete it.
...
%%
%% The volume must be in the `available' state (not attached to an
%% instance).
%%
...
%%
%% Before you can delete an endpoint service configuration, you must reject
...
%% delete the VPC peering connection if it's in the `active' state.
%% The owner of the requester VPC can delete a VPC peering connection in the
...
%%
%% You must first detach the virtual private gateway from the VPC. Note that
%% you don't need to delete the virtual private gateway if you plan to
%% delete and recreate the VPN connection between your VPC and your network.
...
%% doesn't affect the files that you uploaded to Amazon S3 when you
%% created the AMI.
...
%%
%% <ul> <li> `default-vpc': The ID of the default VPC for your account,
%% or `none'.
%%
...
%%
%% </li> <li> `supported-platforms': This attribute is deprecated.
%%
%% </li> <li> `vpc-max-elastic-ips': The maximum number of Elastic IP
%% addresses that you can allocate.
%%
%% </li> <li> `vpc-max-security-groups-per-interface': The maximum number
%% of security groups that you can assign to a network interface.
%%
%% </li> </ul>
...
%% address ranges, use `DescribePublicIpv4Pools' or
%% `DescribeIpv6Pools'.
...
%%
%% The results describe only the Capacity Reservations in the Amazon Web
...
%%
%% These include public AFIs, private AFIs that you own, and AFIs owned by
%% other Amazon Web Services accounts for which you have load permissions.
...
%% override the settings by running the `ModifyIdFormat' command.
...
%% specify an instance ID that is not valid, such as an instance that is not
%% a burstable performance instance, an error is returned.
%%
...
%% == Limitations ==
%%
%% <ul> <li> Supported zones
%%
%% <ul> <li> Availability Zone
%%
%% </li> <li> Local Zone
%%
%% </li> </ul> </li> <li> Supported instance types
%%
%% <ul> <li> `hpc6a.48xlarge' | `hpc6id.32xlarge' |
%% `hpc7a.12xlarge' | `hpc7a.24xlarge' | `hpc7a.48xlarge' |
%% `hpc7a.96xlarge' | `hpc7g.4xlarge' | `hpc7g.8xlarge' |
%% `hpc7g.16xlarge'
%%
%% </li> <li> `p3dn.24xlarge' | `p4d.24xlarge' | `p4de.24xlarge'
%% | `p5.48xlarge'
%%
...
%%
%% The results can be filtered by location (Region or Availability Zone). If
%% no location is specified, the instance types offered in the current Region
%% are returned.
...
%%
%% You can describe all versions, individual versions, or a range of
%% versions. You can also describe all the latest versions or all the default
...
%%
%% By default, all local gateway route tables are described. Alternatively,
...
%%
%% By default, all local gateways are described. Alternatively, you can
%% filter the results.
...
%% and IAM users that have explicitly specified a longer ID (17-character ID)
%% preference.
%%
...
%% Services accounts have create volume permissions for these snapshots.
%%
%% </li> <li> explicit: The owner of the snapshot granted create volume
%% permissions to a specific Amazon Web Services account.
%%
%% </li> <li> implicit: An Amazon Web Services account has implicit create
%% volume permissions for all snapshots it owns.
%%
%% </li> </ul> The list of snapshots returned can be filtered by specifying
...
%% Web Services account IDs (if you own the snapshots), `self' for
...
%%
%% You can describe the store tasks for specified AMIs. If you don't
%% specify the AMIs, you get a paginated list of store tasks from the last 31
%% days.
%%
...
%%
%% By default, all transit gateway route tables are described. Alternatively,
%% you can filter the results.
...
%%
%% By default, all VPC attachments are described. Alternatively, you can
%% filter the results.
...
%%
%% Volume status provides the result of the checks performed on your volumes
%% to determine events that can impair the performance of your volumes. The
%% performance of a volume can be affected if an issue occurs on the
%% volume's underlying host. If the volume's underlying host
%% experiences a power outage or system issue, after the system is restored,
%% there could be data inconsistencies on the volume. Volume events notify
%% you if this occurs. Volume actions notify you if any action needs to be
...
%% `ok', `impaired' , `warning', or `insufficient-data'. If
...
%% the volume event might be `potential-data-inconsistency'. This means
%% that your volume has been affected by an issue with the underlying host,
%% has all I/O operations disabled, and might have inconsistent data.
%%
...
%% it's linked. Similarly, the DNS hostname of an instance in a VPC
...
%% `us-east-1a' and `us-east-1c' and the consumer uses
%% `us-east-1a' and `us-east-1b', the response includes the VPC
...
%% system before detaching the volume. Failure to do so can result in the
%% volume becoming stuck in the `busy' state while detaching. If this
%% happens, detachment can be delayed indefinitely until you unmount the
%% volume, force detachment, reboot the instance, or all three. If an EBS
%% volume is the root device of an instance, it can't be detached while
...
%%
%% You do this if you're planning to turn off the VPC and not use it
%% anymore. You can confirm a virtual private gateway has been completely
%% detached from a VPC by describing the virtual private gateway (any
...
%% process for each new instance. Amazon EC2 must remove all pre-provisioned
%% snapshots before you can enable Windows fast launch again.
%%
...
%%
%% This removes the block public access restriction from your account. With
%% the restriction removed, you can publicly share your AMIs in the specified
...
%% @doc Disables the block public access for snapshots setting at the account
...
%% following happens:
%%
%% <ul> <li> The route that was automatically added for the VPC is deleted
%%
%% </li> <li> All active client connections are terminated
%%
%% </li> <li> New client connections are disallowed
%%
%% </li> <li> The Client VPN endpoint's status changes to
%% `pending-associate'
%%
%% </li> </ul>
...
%%
%% To disassociate the CIDR block, you must specify its association ID. You
%% can get the association ID by using `DescribeVpcs'. You must detach or
%% delete all gateways and resources that are associated with the CIDR block
...
%%
%% To create the optimized Windows image, Amazon EC2 launches an instance and
...
%%
%% The re-enabled AMI is marked as `available' and can be used for
%% instance launches, appears in describe operations, and can be shared.
%% Amazon Web Services accounts, organizations, and Organizational Units that
%% lost access to the AMI when it was disabled do not regain access
%% automatically. Once the AMI is available, it can be shared with them
%% again.
%%
...
%% @doc Enables or modifies the block public access for snapshots setting at
%% the account level for the specified Amazon Web Services Region.
%%
%% After you enable block public access for snapshots in a Region, users can
%% no longer request public sharing for snapshots in that Region. Snapshots
...
%%
%% The Client VPN endpoint configuration file includes the Client VPN
%% endpoint and certificate information clients need to establish a
...
%% key where the certificate, certificate chain, and encrypted private key
%% bundle are stored, and the ARN of the KMS key that's used to encrypt
%% the private key.
...
%% attached to a computer. For Windows instances, the instance console output
...
%% the integration of VPC flow logs with Amazon Athena.
%%
%% This make it easier for you to query and gain insights from VPC flow logs
%% data. Based on the information that you provide, we configure resources in
%% the template to do the following:
%%
%% <ul> <li> Create a table in Athena that maps fields to a custom log format
%%
%% </li> <li> Create a Lambda function that updates the table with new
%% partitions on a daily, weekly, or monthly basis
%%
%% </li> <li> Create a table partitioned between two timestamps in the past
%%
%% </li> <li> Create a set of named queries in Athena that you can use to get
%% started quickly
%%
%% </li> </ul> `GetFlowLogsIntegrationTemplate' does not support
%% integration between Amazon Web Services Transit Gateway Flow Logs and
%% Amazon Athena.
...
%% few minutes. If you try to retrieve the password before it's
%% available, the output returns an empty string. We recommend that you wait
%% up to 15 minutes after launching an instance before trying to retrieve the
%% generated password.
...
%% Instance.
%%
%% If the exchange cannot be performed, the reason is returned in the
%% response. Use `AcceptReservedInstancesExchangeQuote' to perform the
%% exchange.
...
%% be used with the customer gateway device specified for your Site-to-Site
%% VPN connection.
...
%% value for either the `--license-type' or `--usage-operation'
...
%% creates the key pair and gives the keys to you (Amazon Web Services keeps
%% a copy of the public key). With ImportKeyPair, you create the key pair and
...
%% @doc Locks an Amazon EBS snapshot in either governance or compliance mode
%% to protect it against accidental or malicious deletions for a specific
%% duration.
%%
...
%% and lock state:
%%
%% <ul> <li> If the snapshot is locked in governance mode, you can modify the
%% lock mode and the lock duration or lock expiration date.
%%
%% </li> <li> If the snapshot is locked in compliance mode and it is in the
%% cooling-off period, you can modify the lock mode and the lock duration or
%% lock expiration date.
%%
%% </li> <li> If the snapshot is locked in compliance mode and the
%% cooling-off period has lapsed, you can only increase the lock duration or
...
%%
%% You cannot change a Capacity Reservation's instance type, EBS
...
%% strategy is `lowest-price', the EC2 Fleet terminates the instances
...
%% Spot Instance pools that have the least available Spot Instance capacity.
%% If the allocation strategy is `diversified', the EC2 Fleet terminates
%% instances across the Spot Instance pools. Alternatively, you can request
%% that the EC2 Fleet keep the fleet at its current size, but not replace any
...
%%
%% When auto-placement is enabled, any instances that you launch with a
...
%% `image' | `import-task' | `internet-gateway' |
...
%% `security-group' | `subnet' | `subnet-cidr-block-association'
...
%%
%% </li> <li> Change the Dedicated Host with which an instance is associated.
%%
...
%% tenancy can be modified in the same request.
%%
...
%% version. When launching an instance, the default version applies when a
%% launch template version is not specified.
...
%% attach and detach security groups from an existing EC2 instance.
...
%% snapshot's list of create volume permissions, but you cannot do both
%% in a single operation. If you need to both add and remove account IDs for
...
%% `capacityOptimized', the Spot Fleet terminates the instances in the
%% Spot Instance pools that have the least available Spot Instance capacity.
%% If the allocation strategy is `diversified', the Spot Fleet terminates
%% instances across the Spot Instance pools. Alternatively, you can request
...
%%
%% <ul> <li> To modify a subnet on an Outpost rack, set both
%% `MapCustomerOwnedIpOnLaunch' and `CustomerOwnedIpv4Pool'. These
%% two parameters act as a single attribute.
%%
%% </li> <li> To modify a subnet on an Outpost server, set either
...
%% you no longer want to mirror network services, use
...
%% including volume size, volume type, and IOPS capacity.
%%
...
%% the volume is in the `in-use' or `available' state before you can
...
%%
%% You can change the Network Load Balancers or Gateway Load Balancers for
...
%%
%% You can add or remove permissions for service consumers (Amazon Web
%% Services accounts, users, and IAM roles) to connect to your endpoint
%% service.
%%
...
%%
%% You can change the instance tenancy attribute of a VPC to `default'
...
%%
%% To modify the target gateway, the following migration options are
%% available:
%%
%% <ul> <li> An existing virtual private gateway to a new virtual private
%% gateway
%%
%% </li> <li> An existing virtual private gateway to a transit gateway
%%
%% </li> <li> An existing transit gateway to a new transit gateway
%%
%% </li> <li> An existing transit gateway to a virtual private gateway
%%
%% </li> </ul> Before you perform the migration to the new gateway, you must
%% configure the new gateway. Use `CreateVpnGateway' to create a virtual
%% private gateway, or `CreateTransitGateway' to create a transit
%% gateway.
%%
...
%% Site-to-Site VPN connection. Use `CreateTransitGatewayRoute' to add
%% the routes.
%%
...
%% status changes from `pending-provision' to `provisioned'. To
...
%% `AllocateAddress' with either the specific address from the address
%% pool or the ID of the address pool.
...
%%
%% You must have active Dedicated Hosts in your account before you purchase a
%% reservation. This action results in the specified reservation being
...
%% @doc You can no longer purchase Scheduled Instances.
%%
...
%% specified instances. The operation succeeds if the instances are valid and
...
%% from a snapshot of a root device volume. You specify the snapshot using a
...
%%
%% <ol> <li> Launch an instance from an existing AMI with that billing
%% product code.
%%
%% </li> <li> Customize the instance.
%%
%% </li> <li> Create an AMI from the instance using `CreateImage'.
%%
%% </li> </ol> If you purchase a Reserved Instance to apply to an On-Demand
%% Instance that was launched from an AMI with a billing product code, make
%% sure that the Reserved Instance has the matching billing product code. If
...
%% The VPC attachment must be in the `pendingAcceptance' state. Use
%% `DescribeTransitGatewayVpcAttachments' to view your pending VPC
%% attachment requests. Use `AcceptTransitGatewayVpcAttachment' to accept
%% a VPC attachment request.
...
%%
%% The VPC peering connection must be in the `pending-acceptance' state.
%% Use the `DescribeVpcPeeringConnections' request to view your
...
%% it from any instance that it's associated with. To disassociate an
...
%% specified in another request, for example, to modify the host. You must
%% stop or terminate all instances on a host before it can be released.
%%
...
%%
%% The instance must be in the `running' state. If your experience with
%% the instance differs from the instance status returned by
...
%% this Region to the Amazon Web Services managed KMS key for EBS.
%%
...
%%
%% To reset the `kernel' or `ramdisk', the instance must be in a
%% stopped state. To reset the `sourceDestCheck', the instance can be
...
%% rule's values, an `InvalidPermission.NotFound' client error is
...
%%
%% <ul> <li> If you don't specify a subnet ID, we choose a default subnet
%% from your default VPC for you. If you don't have a default VPC, you
%% must specify a subnet ID in the request.
%%
%% </li> <li> All instances have a network interface with a primary private
%% IPv4 address. If you don't specify this address, we choose one from
...
%%
%% For instances based on Intel and AMD processors, the interrupt is received
...
%%
%% If the path is reachable, the operation returns the shortest feasible
%% path.
...
%%
%% This operation is idempotent; if you terminate an instance more than once,
%% each call succeeds.
%%
...
%%
%% <ul> <li> The specified instances that are in the same Availability Zone
%% as the protected instance are not terminated.
%%
%% </li> <li> The specified instances that are in different Availability
%% Zones, where no other specified instances are protected, are successfully
%% terminated.
%%
%% </li> </ul> For example, say you have the following instances:
%%
%% <ul> <li> Instance A: `us-east-1a'; Not protected
%%
%% </li> <li> Instance B: `us-east-1a'; Not protected
%%
%% </li> <li> Instance C: `us-east-1b'; Protected
%%
%% </li> <li> Instance D: `us-east-1b'; not protected
%%
%% </li> </ul> If you attempt to terminate all of these instances in the same
%% request, the request reports failure with the following results:
%%
%% <ul> <li> Instance A and Instance B are successfully terminated because
%% none of the specified instances in `us-east-1a' are enabled for
%% termination protection.
%%
%% </li> <li> Instance C and Instance D fail to terminate because at least
%% one of the specified instances in `us-east-1b' (Instance C) is enabled
%% for termination protection.
%%
%% </li> </ul> Terminated instances remain visible after termination (for
...
%% locked in compliance mode but still in the cooling-off period.
%%
%% You can't unlock a snapshot that is locked in compliance mode after
