...
%% metrics, which are the variables you want to measure for your resources
%% and
%% applications.
%%
%% CloudWatch alarms send notifications or automatically change the resources
%% you are monitoring based on rules
%% that you define. For example, you can monitor the CPU usage and disk reads
%% and writes of your Amazon EC2
...
%% your own custom metrics. With CloudWatch, you gain system-wide visibility
%% into resource
...
%% include no more
%% than one composite alarm. For example, you could delete 99 metric alarms
%% and one composite alarms with one operation, but you can't
%% delete two composite alarms with one operation.
%%
%% If you specify an incorrect alarm name or make any other error in the
%% operation, no alarms are
...
%% composite alarm B also depends on composite alarm A. In this scenario, you
%% can't delete any composite alarm that is part of the cycle
%% because there is always still a composite alarm that depends on that alarm
%% that you want to delete.
%%
%% To get out of such a situation, you must
%% break the cycle by changing the rule of one of the composite alarms in the
%% cycle to remove a dependency that creates the cycle. The simplest
%% change to make to break a cycle is to change the `AlarmRule' of one of
%% the alarms to `false'.
%%
...
%% If an alarm name is not specified, the histories for either all metric
%% alarms or all composite alarms are returned.
%%
%% CloudWatch retains the history of an alarm even if you delete the alarm.
%%
%% To use this operation and return information about a composite alarm, you
%% must be signed on with
%% the `cloudwatch:DescribeAlarmHistory' permission that is scoped to
...
%% must be signed on with
%% the `cloudwatch:DescribeAlarms' permission that is scoped to `*'.
%% You can't return information
...
%% the specified metric. It does not return alarms based on math expressions
%% that
%% use the specified metric, or composite alarms that use the specified
%% metric.
-spec describe_alarms_for_metric(map(), describe_alarms_for_metric_input()) ->
...
-spec describe_alarms_for_metric(map(), describe_alarms_for_metric_input(), proplists:proplist()) ->
    {ok, describe_alarms_for_metric_output(), tuple()} |
    {error, any()}.
...
-spec disable_alarm_actions(map(), disable_alarm_actions_input()) ->
    {ok, undefined, tuple()} |
    {error, any()}.
...
-spec disable_alarm_actions(map(), disable_alarm_actions_input(), proplists:proplist()) ->
    {ok, undefined, tuple()} |
    {error, any()}.
...
-spec enable_alarm_actions(map(), enable_alarm_actions_input()) ->
    {ok, undefined, tuple()} |
    {error, any()}.
...
-spec enable_alarm_actions(map(), enable_alarm_actions_input(), proplists:proplist()) ->
    {ok, undefined, tuple()} |
    {error, any()}.
...
%%
%% The operation
%% can also include a CloudWatch Metrics Insights query, and one or more
%% metric math functions.
%%
%% A `GetMetricData' operation that does not include a query can retrieve
%% as many as 500 different
...
%% new time series that represent new insights into your data. For example,
%% using Lambda
...
%% query. But the same `GetMetricData' operation can also retrieve other
%% metrics. Metrics Insights queries
%% can query only the most recent three hours of metric data. For more
%% information about Metrics Insights,
...
%% hours. These data points are high-resolution
...
%% After 63 days, the data is further aggregated and is available with
...
%% same expression, but
%% you can reference results from a Metrics Insights query within other
%% Metric math expressions. A Metrics Insights
...
%% single time series. A Metrics Insights
...
%% documents.
%% You could also retrieve images regularly, such as every minute, and create
%% your own
...
-spec get_metric_widget_image(map(), get_metric_widget_image_input()) ->
...
-spec get_metric_widget_image(map(), get_metric_widget_image_input(), proplists:proplist()) ->
    {ok, get_metric_widget_image_output(), tuple()} |
    {error, any()}.
...
%% those dashboards with names starting with the prefix are listed.
%% Otherwise, all dashboards in your account are
%% listed.
%%
...
%% appear. To see metric
%% statistics sooner, use GetMetricData:
%% https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricData.html
%% or GetMetricStatistics:
%% https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricStatistics.html.
%%
...
%% include as many as 100 underlying alarms.
...
%% the `cloudwatch:PutCompositeAlarm' permission that is scoped to
%% `*'. You can't create a
%% composite alarms if your `cloudwatch:PutCompositeAlarm' permission has
%% a narrower scope.
%%
...
%% and then use the View/edit source command in the Actions menu to display
%% the JSON block
...
%% dashboard was created by script and should
%% not be changed in the console. This message could also point console users
%% to the location
...
%% log events in that log group. For more information,
...
%% a Metrics Insights query for an alarm, see Create alarms on Metrics
%% Insights queries:
...
%% Amazon Web Services Management Console, the CLI, or by using the
%% PutMetricAlarm API, CloudWatch
...
%% already have a sharing role named CloudWatch-CrossAccountSharingRole. If
%% it does not already
...
%% CloudWatch to assume the sharing role in the sharing account. If it does
%% not, you must create it following the directions in Set up a
...
%% the metric collects. Each dimension
...
%% specify
%% time stamps that are as much as two weeks before the current date, and as
%% much as 2 hours after
...
%% https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricStatistics.html
%% from the time they
%% are submitted. Data points with time stamps between 3 and 24 hours ago can
%% take as much as 2 hours to become available
%% for for GetMetricData:
%% https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricData.html
%% or
%% GetMetricStatistics:
%% https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricStatistics.html.
...
%% metric stream in a monitoring account,
%% you can choose whether to include metrics from source accounts in the
...
%% to send an
%% Amazon SNS message when an alarm is triggered, temporarily changing the
%% alarm state to
%% `ALARM' sends an SNS message.
%%
%% Metric alarms
%% returns to their actual state quickly, often within seconds. Because the
%% metric alarm state change
%% happens quickly, it is typically only visible in the alarm's History
%% tab in the Amazon CloudWatch console or through
...
%% returns to its actual state only once any of its children alarms change
%% state. It is also
