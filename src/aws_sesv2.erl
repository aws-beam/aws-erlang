...
create_contact(Client, ContactListName, Input0, Options) ->
    Method = post,
    Path = ["/v2/email/contact-lists/", http_uri:encode(ContactListName), "/contacts"],
    SuccessStatusCode = undefined,
...
delete_contact(Client, ContactListName, EmailAddress, Input0, Options) ->
    Method = delete,
    Path = ["/v2/email/contact-lists/", http_uri:encode(ContactListName), "/contacts/", http_uri:encode(EmailAddress), ""],
    SuccessStatusCode = undefined,
...
delete_contact_list(Client, ContactListName, Input0, Options) ->
    Method = delete,
    Path = ["/v2/email/contact-lists/", http_uri:encode(ContactListName), ""],
    SuccessStatusCode = undefined,
...
get_import_job(Client, JobId, Options)
  when is_map(Client), is_list(Options) ->
    Path = ["/v2/email/import-jobs/", http_uri:encode(JobId), ""],
    SuccessStatusCode = undefined,
...
list_custom_verification_email_templates(Client, NextToken, PageSize)
  when is_map(Client) ->
    list_custom_verification_email_templates(Client, NextToken, PageSize, []).
list_custom_verification_email_templates(Client, NextToken, PageSize, Options)
  when is_map(Client), is_list(Options) ->
    Path = ["/v2/email/custom-verification-email-templates"],
    SuccessStatusCode = undefined,
...
%%
%% You can execute this operation no more than once per second.
list_email_templates(Client, NextToken, PageSize)
  when is_map(Client) ->
    list_email_templates(Client, NextToken, PageSize, []).
list_email_templates(Client, NextToken, PageSize, Options)
  when is_map(Client), is_list(Options) ->
    Path = ["/v2/email/templates"],
    SuccessStatusCode = undefined,
...
list_suppressed_destinations(Client, EndDate, NextToken, PageSize, Reasons, StartDate)
  when is_map(Client) ->
    list_suppressed_destinations(Client, EndDate, NextToken, PageSize, Reasons, StartDate, []).
list_suppressed_destinations(Client, EndDate, NextToken, PageSize, Reasons, StartDate, Options)
  when is_map(Client), is_list(Options) ->
