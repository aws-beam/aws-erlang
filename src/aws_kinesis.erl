...
%% Data Streams supports
%% large records up to 10 MiB in size, but the average throughput for your
%% stream cannot exceed
%% 1 MiB per second. For more information about how large records are
%% handled, see
%% Large records:
%% https://docs.aws.amazon.com/streams/latest/dev/large-records.html.
...
%% stream. Use this operation to scale your stream to a specified throughput
%% level before anticipated traffic spikes, or to release excess capacity
%% after traffic has decreased.
%%
%% When invoking this API, you must use either the `StreamARN' or the
%% `StreamName' parameter, or both. It is recommended that you use the
%% `StreamARN' input parameter when you invoke this API.
...
do_request(Client, Action, Input0, Options) ->
    ...
    {URL, Host} = aws_util:apply_endpoint_url_override(build_url(DefaultHost, Client1), DefaultHost, <<"/">>, <<"AWS_ENDPOINT_URL_KINESIS">>),
    Headers = [
        {<<"Host">>, Host},
