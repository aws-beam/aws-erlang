...
%%
%% </li> <li> The compression formats `SNAPPY' or `ZIP' cannot be specified
...
%% to `ENABLING', and then to `ENABLED'. The encryption status of a delivery
...
%% This operation is asynchronous. It returns immediately. When you invoke
%% it, Kinesis Data Firehose first sets the encryption status of the stream
%% to `DISABLING', and then to `DISABLED'. You can continue to read and write
