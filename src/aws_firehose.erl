...
%% from providers using `PutRecord' or `PutRecordBatch', or it can be
%% configured to use an existing Kinesis stream as its source. To specify a
%% Kinesis data stream as input, set the `DeliveryStreamType' parameter
%% to `KinesisStreamAsSource', and provide the Kinesis stream Amazon
...
%% intermediate location. Firehose first delivers data to Amazon S3 and then
...
%% the `CREATING' state. To check the state of a delivery stream, use
...
%% and it goes into the `DELETING' state.While the delivery stream is in
%% the `DELETING' state, the service might continue to accept records,
%% but it doesn't make any guarantees with respect to delivering the
%% data. Therefore, as a best practice, first stop any applications that are
...
%%
%% If `PutRecordBatch' throws `ServiceUnavailableException', the API
...
%% `ENABLING', and then to `ENABLED'. The encryption status of a
...
%% is of type `CUSTOMER_MANAGED_CMK', Firehose schedules the grant it had
%% on the old CMK for retirement. If the new CMK is of type
