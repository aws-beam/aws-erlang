...
%% @doc Parallel Computing Service (PCS) is a managed service that makes it
%% easier for you to run and scale your high performance computing (HPC)
%% workloads, and build scientific and engineering models on Amazon Web
%% Services using Slurm.
%%
%% For more information, see the Parallel Computing Service User Guide:
%% https://docs.aws.amazon.com/pcs/latest/userguide.
%%
...
%%
%% The API actions operate on PCS resources. A resource is an entity in
...
%% information about resources in Amazon Web Services, see Resource:
...
%%
%% An PCS compute node is an Amazon EC2 instance. You don't launch
%% compute nodes directly. PCS uses configuration information that you
%% provide to launch compute nodes in your Amazon Web Services account. You
%% receive billing charges for your running compute nodes. PCS automatically
%% terminates your compute nodes when you delete the PCS resources related to
%% those compute nodes.
-module(aws_pcs).
...
%% controller communicates with the cluster resources in your account. The
%% subnets and security groups for the cluster must already exist before you
%% use this API action.
%%
...
%%
%% You associate a compute node group with a cluster through 1 or more PCS
%% queues or as part of the login fleet. A compute node group includes the
%% definition of the compute properties and lifecycle management. PCS uses
...
%% with any other queues, PCS terminates all the compute nodes for this
%% queue.
...
%% @doc Returns a list of all tags on an PCS resource.
...
%% @doc Adds or edits tags on an PCS resource.
%%
...
%% @doc Deletes tags from an PCS resource.
%%
...
do_request(Client, Action, Input0, Options) ->
    ...
    {URL, Host} = aws_util:apply_endpoint_url_override(build_url(DefaultHost, Client1), DefaultHost, <<"/">>, <<"AWS_ENDPOINT_URL_PCS">>),
    Headers = [
        {<<"Host">>, Host},
