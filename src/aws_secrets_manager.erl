...
%% `AccessDeniedException' while attempting to retrieve any of the
%% secrets,
%% you can see the errors in `Errors' in the response.
%%
%% Secrets Manager generates CloudTrail
%% `GetSecretValue' log entries for each secret you request when you call
%% this
...
%% you
%% must also have `secretsmanager:ListSecrets'. If the secrets are
%% encrypted
...
%% `ListSecretVersionIds'. Then use `UpdateSecretVersionStage' to
...
%% attaches the staging label
%% `AWSCURRENT' to it.
%%
%% For database credentials you want to rotate, for Secrets Manager to be
%% able to rotate the
%% secret, you must make sure the JSON you store in the `SecretString'
%% matches
%% the JSON
...
%% secret, you also need `secretsmanager:TagResource'. To add replica
%% Regions,
...
%% which you can restore the secret. The minimum recovery window is 7 days.
%% The default
...
%% all staging
%% labels from the version using `UpdateSecretVersionStage'. This marks
%% the
%% version as deprecated, and then Secrets Manager can automatically delete
%% the version in the
%% background.
%%
...
%% using a customer-managed key instead of the Amazon Web Services managed
%% key
%% `aws/secretsmanager', then you also need `kms:Decrypt'
...
%% @doc Creates a new version of your secret by creating a new encrypted
%% value and attaching
%% it to the secret.
%%
...
%% secret versions created in the last 24 hours. If you call
%% `PutSecretValue'
%% more than once every 10 minutes, you will create more versions than
%% Secrets Manager removes, and
...
%% attaches the staging label `AWSCURRENT' to it. If this operation moves
%% the
%% staging label `AWSCURRENT' from another version to this version, then
%% Secrets Manager
%% also automatically moves the staging label `AWSPREVIOUS' to the
%% version that
...
%% However, if
%% the secret data is different, then the operation fails because you
%% can't modify an
...
%% secret is encrypted with a KMS key other than `aws/secretsmanager',
%% you also
...
%% rotation. If you don't include the configuration parameters, the
%% operation starts a
...
%% not be
%% attached to any version. If the `AWSPENDING' staging label is present
%% but not
%% attached to the same version as `AWSCURRENT', then any later
%% invocation of
...
%% the secret's metadata. They are not associated with specific versions
%% of the secret.
...
%% Manager removes outdated versions when
...
%% `AWSCURRENT' to
%% the new version. Then it attaches the label `AWSPREVIOUS' to the
%% version that
%% `AWSCURRENT' was removed from.
%%
...
%% you change the KMS key and you don't have `kms:Encrypt' permission
%% to the new
...
%% then the
%% version is considered to be 'deprecated' and can be deleted by
%% Secrets Manager.
%%
%% Secrets Manager generates a CloudTrail log entry when you call this
%% action. Do not include sensitive information in request parameters because
...
%% an automated reasoning engine, to ensure your resource
%% policy does not allow broad access to your secret, for example policies
%% that use
...
do_request(Client, Action, Input0, Options) ->
    ...
    {URL, Host} = aws_util:apply_endpoint_url_override(build_url(DefaultHost, Client1), DefaultHost, <<"/">>, <<"AWS_ENDPOINT_URL_SECRETS_MANAGER">>),
    Headers = [
        {<<"Host">>, Host},
