...
%% applies to Amazon S3 read operations when `GetQueryResults' is called. If
%% an expected bucket owner has been specified and the query results are in
%% an Amazon S3 bucket whose owner account ID is different from the expected
%% bucket owner, the `GetQueryResults' call fails with an Amazon S3
%% permissions error.
%%
