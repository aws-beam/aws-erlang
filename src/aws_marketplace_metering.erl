...
%% CloudTrail to verify that the software as a service (SaaS) metering
%% records that
...
%% `BatchMeterUsage' does not support `CustomerIdentifier' for new
%% integrations. Existing integrations continue to work. Review the new
%% integration for Concurrent Agreements here:
%% https://catalog.workshops.aws/mpseller/en-US/saas/integration-for-concurrent-agreements.
%% For additional implementation details, see BatchMeterUsage code example
%% with LicenseArn:
%% https://docs.aws.amazon.com/marketplace/latest/userguide/saas-code-examples.html#saas-batchmeterusage-licensearn-example
...
%% accept
%% usage records for the previous billing month until 06:00 UTC on the first
%% day of the
%% next month. For example, you must submit March usage records before 06:00
%% UTC on
%% April 1. On April 1 at 05:00 UTC, you can still submit records for March
%% 31 (within the 6-hour grace period). After 06:00 UTC on April 1, March
%% records are rejected regardless of the normal 24-hour submission window.
%% After this grace period, we return a
...
%% @doc As a seller, your software hosted in the buyer's Amazon Web
%% Services account uses this API action to emit metering records directly to
%% Amazon Web Services Marketplace.
%%
%% You must use the following buyer Amazon Web Services account credentials
%% to sign the API request.
%%
...
%% EKS Pod Identity:
%% https://docs.aws.amazon.com/eks/latest/userguide/pod-identities.html, the
%% node role, or long-term access keys is not supported.
%%
%% For Amazon ECS deployments, your software must use
%% Amazon ECS task IAM:
%% https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-iam-roles.html
%% role to sign the API call for the `MeterUsage' API operation. Using
%% the node role or long-term access keys are not supported.
%%
...
%% to sign the API call for the `MeterUsage' API operation. Long-term
...
%% can't modify values
%% after they're recorded. If you report usage before a current hour
%% ends, you will be unable to report additional usage until the next hour
%% begins.
%% The `Timestamp' request parameter is rounded down to the hour and used
%% to enforce this once-per-hour rule for idempotency.
%% For requests that are identical after the `Timestamp' is rounded down,
%% the API is idempotent and returns the metering record ID.
%%
%% For Amazon Bedrock AgentCore Runtime deployments, you can report usage
%% multiple times per hour for the same dimension.
%% You do not need to aggregate metering records by the hour. You must
%% include an idempotency token in the `ClientToken' request parameter.
%% If using an Amazon
%% SDK or the Amazon Web Services CLI, you must use the latest version which
%% automatically includes an idempotency token in the `ClientToken'
%% request parameter so that the request is processed successfully.
%% The `Timestamp' request parameter is not rounded down to the hour and
%% is not used for duplicate validation. Requests with duplicate
%% `Timestamps' are aggregated as long as the
%% `ClientToken' is unique.
%%
%% If you submit records more than six hours after events occur, the records
%% won't be accepted. The timestamp in your request determines when an
%% event is recorded.
%%
...
do_request(Client, Action, Input0, Options) ->
    ...
    {URL, Host} = aws_util:apply_endpoint_url_override(build_url(DefaultHost, Client1), DefaultHost, <<"/">>, <<"AWS_ENDPOINT_URL_MARKETPLACE_METERING">>),
    Headers = [
        {<<"Host">>, Host},
