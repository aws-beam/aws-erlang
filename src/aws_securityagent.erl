...
%%   <<"pentestJobId">> => [string()],
%%   <<"reasoning">> => [string()],
%%   <<"revalidationJobIds">> => list([string()]()),
%%   <<"riskLevel">> => list(any()),
%%   <<"riskScore">> => [string()],
%%   <<"riskType">> => [string()],
%%   <<"status">> => list(any()),
...
%% }
-type report_filters() :: #{binary() => any()}.
...
do_request(Client, Method, Path, Query, Headers0, Input, Options, SuccessStatusCode) ->
    ...
    {URL1, Host} = aws_util:apply_endpoint_url_override(URL0, DefaultHost, PathBin, <<"AWS_ENDPOINT_URL_SECURITYAGENT">>),
