...
%% uses standardized product attributes such as <code>Location</code>,
%% <code>Storage Class</code>, and <code>Operating System</code>, and
%% provides prices at the SKU level. You can use the AWS Price List Service
...
%%
%% Use <code>GetServices</code> without a service code to retrieve the
%% service codes for all AWS services, then <code>GetServices</code> with a
%% service code to retreive the attribute names for that service. After you
...
%% all services. Use this without a service code to get the service codes for
%% all services. Use it with a service code, such as <code>AmazonEC2</code>,
%% to get information specific to that service, such as the attribute names
...
%% @doc Returns a list of attribute values. Attibutes are similar to the
