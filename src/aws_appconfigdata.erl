...
%% configuration
%% session using the AppConfig Data `StartConfigurationSession' API
%% action.
%% Your session's client then makes periodic calls to
...
%% before
%% making its next call to `GetLatestConfiguration'. This duration may
%% vary
...
do_request(Client, Method, Path, Query, Headers0, Input, Options, SuccessStatusCode) ->
    ...
    {URL1, Host} = aws_util:apply_endpoint_url_override(URL0, DefaultHost, PathBin, <<"AWS_ENDPOINT_URL_APPCONFIGDATA">>),
