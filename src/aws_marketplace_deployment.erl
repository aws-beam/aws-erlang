...
%% @doc The AWS Marketplace Deployment Service supports the Quick Launch
...
-export([list_tags_for_resource/2,
         ...
         put_deployment_parameter/4,
         put_deployment_parameter/5,
         tag_resource/3,
         tag_resource/4,
         untag_resource/3,
         untag_resource/4]).
...
%%   <<"secretString">> => string()
%% }
-type deployment_parameter_input() :: #{binary() => any()}.
...
%%   <<"agreementId">> := string(),
%%   <<"clientToken">> => string(),
%%   <<"deploymentParameter">> := deployment_parameter_input(),
%%   <<"expirationDate">> => [non_neg_integer()],
%%   <<"tags">> => map()
%% }
-type put_deployment_parameter_request() :: #{binary() => any()}.
...
%%   <<"tags">> => map()
%% }
-type put_deployment_parameter_response() :: #{binary() => any()}.
...
put_deployment_parameter(Client, Catalog, ProductId, Input0, Options0) ->
    Method = post,
    Path = ["/catalogs/", aws_util:encode_uri(Catalog), "/products/", aws_util:encode_uri(ProductId), "/deployment-parameters"],
    SuccessStatusCode = 200,
    {SendBodyAsBinary, Options1} = proplists_take(send_body_as_binary, Options0, false),
    {ReceiveBodyAsBinary, Options2} = proplists_take(receive_body_as_binary, Options1, false),
    Options = [{send_body_as_binary, SendBodyAsBinary},
...
do_request(Client, Method, Path, Query, Headers0, Input, Options, SuccessStatusCode) ->
    ...
    {URL1, Host} = aws_util:apply_endpoint_url_override(URL0, DefaultHost, PathBin, <<"AWS_ENDPOINT_URL_MARKETPLACE_DEPLOYMENT">>),
