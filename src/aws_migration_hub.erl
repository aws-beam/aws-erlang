...
%%
%% <ul> <li> Migration tools can call the
%% <code>AssociateCreatedArtifact</code> operation to indicate which AWS
...
%% was previously created as an AWS resource used for access control. This
%% API has the following traits:
%%
%% <ul> <li> The only parameter needed for
...
%% on a <code>ListProgressUpdateStreams</code> call.
%%
%% </li> <li> <code>CreateProgressUpdateStream</code>,
%% <code>ImportMigrationTask</code>, <code>NotifyMigrationTaskState</code>,
...
%% API has the following traits:
%%
%% <ul> <li> A migration user can call the
%% <code>DisassociateCreatedArtifacts</code> operation to disassociate a
...
%% @doc Lists all the migration statuses for your applications. If you use
%% the optional <code>ApplicationIds</code> parameter, only the migration
...
%% associated by the <code>AssociateCreatedArtifact</code> API.
%%
...
%% with the user account making this call. This API has the following traits:
%%
...
%% @doc Sets the migration state of an application. For a given application
...
%%
%% <ul> <li> Migration tools will call the
%% <code>NotifyMigrationTaskState</code> API to share the latest progress and
%% status.
%%
%% </li> <li> <code>MigrationTaskName</code> is used for addressing updates
%% to the correct target.
%%
%% </li> <li> <code>ProgressUpdateStream</code> is used for access control
%% and to provide a namespace for each migration tool.
%%
%% </li> </ul>
...
%% it can be associated in the Application Discovery Service repository. This
...
%% example, if it is first called with a MAC address, but later, it is
%% desired to <i>add</i> an IP address, it will then be required to call it
%% with <i>both</i> the IP and MAC addresses to prevent overriding the MAC
%% address.
%%
%% </li> <li> Note the instructions regarding the special use case of the <a
...
%% will always return 200, whether an association occurs or not. To confirm
%% if an association was found based on the provided details, call
%% <code>ListDiscoveredResources</code>.
%%
%% </note>
