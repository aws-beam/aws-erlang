...
%% Protocol
%% (OCSP) and/or a certificate revocation list (CRL), the CA type, and an
%% optional
%% idempotency token to avoid accidental creation of multiple CAs. The CA
%% configuration
%% specifies the name of the algorithm and key size to be used to create the
%% CA private
...
%% for the S3
%% bucket that is included in certificates issued by the CA. If successful,
%% this action
...
%% renew ACM certificates that reside in the same Amazon Web Services account
%% as the CA.
%%
...
%% action and
%% revoke them with the DeletePermission:
...
%% Name (ARN) of the private CA that you want to delete. You can find the ARN
%% by calling
...
%% (that is,
%% the status of the CA is `CREATING'). You can also delete it if the CA
%% has
%% been created but you haven't yet imported the signed certificate into
%% Amazon Web Services Private CA (that
...
%% `DELETED'. However, the CA won't be permanently deleted until the
%% restoration
...
%% reside in the
%% same Amazon Web Services account as the CA. If you revoke these
%% permissions, ACM will no longer
...
%% action and
%% listed with the ListPermissions:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_ListPermissions.html
%% action.
%%
%% == About Permissions ==
...
%% an Amazon Web Services Organizations unit. Policies are under the control
%% of a CA administrator. For more information,
...
%% the ACM service to assume the identity of the user, subject to
...
%% in policies. For more information,
...
%% The output contains the status of your CA. This can be any of the
%% following:
%%
...
%% key is used
%% when you call the IssueCertificate:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_IssueCertificate.html
%% action or the
%% RevokeCertificate:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_RevokeCertificate.html
...
%%
%% The
%% ARN of the certificate is returned when you call the IssueCertificate:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_IssueCertificate.html
%% action. You
...
%% action. Sign the CSR with your Amazon Web Services Private CA-hosted
%% or on-premises root or subordinate CA. Then import the signed certificate
%% back into
...
%% are using a chain of trust whose root is located outside Amazon Web
%% Services Private CA. Before you can call
...
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_CreateCertificateAuthority.html
%% action to create the private CA that you
...
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_CreatePermission.html
%% action and
%% revoked with the DeletePermission:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_DeletePermission.html
%% action.
%%
%% == About Permissions ==
...
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_DeleteCertificateAuthority.html
%% action. Currently, you can specify 7 to 30 days.
...
%% You can check
%% the time remaining in the restoration period of a private CA in the
%% `DELETED'
...
%% returns. To change its status to
...
%% revocation list (CRL) when you create or update your private CA,
%% information about the
%% revoked certificates will be included in the CRL. Amazon Web Services
%% Private CA writes the CRL to an S3
%% bucket that you specify. A CRL is typically updated approximately 30
%% minutes after a
...
%% and organize your Amazon Web Services resources. Each tag consists of a
%% key and an optional value. You
%% specify the private CA on input by its Amazon Resource Name (ARN). You
%% specify the tag
%% by using a key-value pair. You can apply a tag to just one private CA if
%% you want to
...
%% CAs. To remove
%% one or more tags, use the UntagCertificateAuthority:
%% https://docs.aws.amazon.com/privateca/latest/APIReference/API_UntagCertificateAuthority.html
...
%% you do not specify the value portion of the tag when calling this action,
%% the tag will
%% be removed regardless of value. If you specify a value, the tag is removed
%% only if it is
