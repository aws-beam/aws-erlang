...
%% Interface (AWS CLI), or by making
...
%% you want to use. As soon as you create a Amazon GameLift Streams
%% application,
...
%% up stream groups and start streams. To track application
...
%% you also scale the number of concurrent streams you want to
...
%% group resource and
%% sets the status to `ACTIVATING'. When the stream group reaches
%% `ACTIVE' status, you can start stream sessions by
%% using StartStreamSession:
...
%% stream session life cycle, when the client disconnects from the stream
%% session, the stream session transitions from `CONNECTED'
%% to `PENDING_CLIENT_RECONNECTION' status. When a client requests to
%% reconnect by calling
%% `CreateStreamSessionConnection', the stream session transitions to
%% `RECONNECTING' status. When the reconnection
%% is successful, the stream session transitions to `ACTIVE' status.
%% After a stream session is disconnected for longer than
...
%% GameLift Streams. However,
%% this does not delete the original files that you uploaded to your Amazon
%% S3 bucket; you can delete these any time after Amazon GameLift Streams
%% creates an
%% application, which is the only time Amazon GameLift Streams accesses your
%% Amazon S3 bucket.
%%
...
%% status is `DELETING'. This operation stops streams in
%% progress and prevents new streams from starting. As a best practice,
%% before deleting the stream group, call ListStreamSessions:
...
%% them. When you delete a stream group, any application associations
...
%% `ACTIVE', `CONNECTED', `PENDING_CLIENT_RECONNECTION', and
%% `RECONNECTING'. You must provide
%% an Amazon Simple Storage Service (Amazon S3) bucket to store the files in.
%% When the session ends, Amazon GameLift Streams produces a compressed
%% folder
%% that contains all of the files and directories that were modified or
%% created by the application during the stream session. AWS uses your
%% security credentials to authenticate and authorize access to your Amazon
%% S3 bucket.
%%
...
%% to retrieve and its
%% stream group ARN. If the operation is successful, it returns properties
...
%% all statuses, in no particular order. You can paginate the results as
%% needed.
...
%% property only shows the `Status' value. To get
...
%% these location. Thus,
%% stream sessions can no longer start from these locations by using this
%% stream group. Amazon GameLift Streams also deletes the content files of
%% all
%% associated applications that were in Amazon GameLift Streams's
%% internal S3 bucket at this location.
%%
...
%% along with the transport protocol and signal request settings
%% to use with the stream. You must have associated at least one application
%% to the stream group before starting a stream session, either
...
%% Amazon GameLift Streams will start a single stream session in the next
...
%% value to the stream
%% session resource and sets the status to `ACTIVATING'. During the
%% stream preparation process, Amazon GameLift Streams queues the request and
%% searches for available stream capacity to run the stream. This can result
...
%% stream connection information. Provide the connection
...
%% Streams
%% stops processing the request, and the stream session object status changes
%% to `ERROR' with status reason
%% `placementTimeout'.
...
%% purposes. You can assign tags to
...
%% can
%% terminate a stream session in any status except `ACTIVATING'. If the
%% stream session is in `ACTIVATING' status, an
%% exception is thrown.
...
%% new values. If the operation is successful, it returns the
...
%% the requested capacity of a stream group per location. If you want to
...
%% Amazon GameLift Streams returns the
