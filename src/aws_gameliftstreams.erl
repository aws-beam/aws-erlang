...
%% tools to upload and configure content for streaming, deploy and scale
%% computing resources
...
%% You can use the API through the
%% Amazon Web Services SDK, the Command Line Interface (AWS CLI), or by
%% making direct REST calls through HTTPS.
%%
...
%% you cannot change the files at a
%% later time.
%%
...
%% `StartStreamSession'. To check the stream group's status, call
%% `GetStreamGroup'.
...
%% reconnect
%% by calling `CreateStreamSessionConnection', the stream session
%% transitions to
...
%% disconnected for longer than `ConnectionTimeoutSeconds', the stream
%% session
%% transitions to the `TERMINATED' status.
%%
%% To connect to an existing stream session, specify the stream group ID and
%% stream
%% session ID that you want to reconnect to, as well as the signal request
%% settings to use
%% with the stream.
%%
...
%% application content files stored with Amazon GameLift Streams. However,
%% this does not delete the
%% original files that you uploaded to your Amazon S3 bucket; you can delete
%% these any time
%% after Amazon GameLift Streams creates an application, which is the only
%% time Amazon GameLift Streams accesses your Amazon S3
%% bucket.
%%
...
%% before deleting the stream group, call `ListStreamSessions' to check
%% for streams in progress and take action to stop them. When you delete a
%% stream group, any
...
%% group that you want to retrieve. If the operation is successful, it
%% returns properties
...
%% status changes to `TERMINATING'. You can terminate a stream session in
%% any
...
%%
%% You
%% can change the description, the set of locations, and the requested
...
%% Name (ARN) and provide the new values. If
%% the request is successful, Amazon GameLift Streams returns the complete
%% updated metadata for the stream
%% group.
