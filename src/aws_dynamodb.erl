...
%% returns a value for `UnprocessedKeys'. You can use this value to retry
...
%% `LastEvaluatedKey' value is also returned and the requestor can use
...
%% `Count' will result in zero. If you did not use a
%% `FilterExpression' in the scan request, then `Count' is the same
%% as `ScannedCount'.
%%
%% `Count' and `ScannedCount' only return the count of items specific
%% to a single scan request and, unless the table is less than 1MB, do not
...
%% consistent read of each item as the scan iterates through the items in the
%% table, you can set the `ConsistentRead' parameter to true. Strong
